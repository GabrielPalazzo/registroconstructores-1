...
export const TomarTramite: React.FC<TomarTramiteProps> = ({
  ...
  const UnLocked = () => {
    return <div onClick={async () => {
      if ((tramite.status==='PENDIENTE DE REVISION')||(tramite.status==='A SUPERVISAR')||(tramite.status==='SUBSANADO') || (tramite.status==='EN REVISION')) {
  ...
  const showComponente = () => tramite.status==='PENDIENTE DE REVISION'|| (tramite.status==='A SUPERVISAR' && getUsuario().isSupervisor()) || (tramite.status=='SUBSANADO' && getUsuario().isBackOffice()) || (tramite.status=='EN REVISION' && getUsuario().isBackOffice())
